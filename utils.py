...
def alm2map(ad, nside):
    ...
    ndims = len(mshape) - 1
    indlist = np.array(mshape[:md.pixaxis] + mshape[md.pixaxis + 1:])
    ...
    for i in range(totnmaps):
        curralms = ad.alms[(currind[:ad.indaxis] + [Ellipsis,] +
                            currind[ad.indaxis:])]
        ...
        lib.alm2map_sc_d(nside, ad.lmax, ad.mmax, convalms, 
                    md.map[currind[:md.pixaxis] 
                + [Ellipsis,] + currind[md.pixaxis:]]) 
        traceind = ndims - 1
